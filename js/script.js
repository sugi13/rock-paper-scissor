...
function DecrementMove(){
    ...
    if(TotalMoves === 0){
        if(userPoint < systemPoint){
          OverAll.innerHTML = "Web Holds Beer!";
        }
        else if(userPoint > systemPoint){
           OverAll.innerHTML = "User Holds Beer!";      
        }
        else if(userPoint == systemPoint){
...
function isUserWon(){
    Userscore.innerHTML = `User: ${userPoint = userPoint + 1}`;
}
function isSystemWon(){
    systemscore.innerHTML = `Web: ${systemPoint = systemPoint + 1}`;
}
